...
## Device Configuration (espbrew.toml)

Device identity is expressed explicitly through an `espbrew.toml` file on the leader rather than by probing every port on startup. This avoids port contention: a board is never probed behind a flash job, so a busy port never stalls flashing, and an undisturbed board can never hold the port.

**Behavior.**
- Auto-probe is disabled by default. A newly connected port is added to cluster state but not probed.
- On startup, the leader reports each configured device as present or absent, and registers a configured device automatically when its port becomes available.
- Discovery runs only on demand, via the CLI (`device discover`) or the `POST /api/v1/devices/discover` endpoint. Each pass is non-blocking and bounded by a timeout, records the ports you ask it to (`--save`), and then stops.
- New mappings are written to `espbrew.toml` by `device add`/`discover --save`, so identity survives restarts through the config file, not only the embedded database.

**Location.** The file is auto-detected in the leader's working directory as `espbrew.toml`, or pointed to with `--config <path>`.
...
| `id` | no | Device ID; defaults to the MAC-derived ID after a probe, or `unprobed-<basename>` when omitted |
| `alias` | no | Human-readable alias; also accepted by `device remove` as a match |
| `chip` | no | Chip type stamped onto the record |
| `description` | no | Free-text description |

**Lifecycle.**

```
1. Port connects        -> added to cluster state (not probed)
2. Leader reads espbrew  -> configured id/chip stamped onto the record
3. device discover       -> on-demand, non-blocking scan of unconfigured ports
4. device discover --save, device add -> mapping written to espbrew.toml
```
...
Device records persist in the embedded database and survive cluster restarts. For stable, probe-free identity the preferred path is the explicit Device Configuration (espbrew.toml) mapping, which is written to `espbrew.toml` by `device add`/`discover --save`. The embedded-database persistence described below is used when devices are managed through the dashboard or when identity is captured from a probe.
...
When devices are unplugged and reconnected, the cluster automatically handles rediscovery. Note that with auto-probe disabled, a freshly connected port is registered without an immediate probe; identity is applied the moment a mapping exists (see Device Configuration (espbrew.toml)) or captured on demand via `device discover`.
