...
func newSlipReader(port serial.Port, ctx context.Context) *slipReader {
	return &slipReader{port: port, ctx: ctx}
}
...
func (r *slipReader) ReadFrame(timeout time.Duration) ([]byte, error) {
	...
	for time.Now().Before(deadline) {
		...
		if r.ctx != nil {
			select {
			case <-r.ctx.Done():
				return nil, fmt.Errorf("slip read cancelled: %w", r.ctx.Err())
			default:
			}
		}
