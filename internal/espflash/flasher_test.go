...
func TestDefaultOptions(t *testing.T) {
	...
	if opts.ConnectAttempts != 9 {
		t.Errorf("ConnectAttempts = %d, want 9", opts.ConnectAttempts)
	}
	if !opts.Compress {
		t.Error("Compress should default to true")
	}
	if opts.Erase {
		t.Error("Erase should default to false")
	}
	if opts.SkipUnchanged {
		t.Error("SkipUnchanged should default to false")
	}
	if opts.FlashMode != "" {
...
func TestSkipUnchangedWithStub(t *testing.T) {
	// Test that SkipUnchanged skips write when MD5 matches
	tests := []struct {
		name          string
		skipUnchanged bool
		stubMode      bool
		...
		{"skip enabled, stub running, MD5 matches", true, true, true, true},
		{"skip enabled, stub running, MD5 differs", true, true, false, false},
		{"skip disabled, stub running", false, true, true, false},
		{"skip enabled, ROM mode", true, false, true, false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			data := []byte{0xE9, 0x00, 0x02, 0x20, 0x00, 0x00, 0x00, 0x00}
			offset := uint32(0x1000)

			mock := &mockConnection{
				stubMode: tt.stubMode,
			}

			// Mock flashMD5 to return matching or different MD5
			if tt.stubMode {
				mock.flashMD5Func = func(addr, size uint32) ([]byte, error) {
					if tt.md5Matches {
						// Return MD5 of test data
						h := md5.Sum(data)
						return h[:], nil
					}
					// Return different MD5
					h := md5.Sum([]byte{0xFF, 0xFF, 0xFF, 0xFF})
					return h[:], nil
				}
			}
			...
			f := &Flasher{
				conn: mock,
				chip: chipDefs[ChipESP32],
				opts: &FlasherOptions{
					SkipUnchanged: tt.skipUnchanged,
				},
			}

			// Track whether flash commands were called
			flashBeginCalled := false
			mock.flashBeginFunc = func(size, offset uint32, encrypted, erase bool) error {
				flashBeginCalled = true
				return nil
			}

			err := f.flashUncompressed(data, offset, nil)
			...
			if tt.shouldSkip {
				...
				if flashBeginCalled {
					t.Error("flashBegin should not be called when skipping unchanged data")
				}
			} else {
				if err != nil && tt.stubMode && !tt.md5Matches {
					// Error is expected for non-stub mode or when mock doesn't implement all methods
					// This is OK for this test - we're checking that the logic proceeds
				}
				if !flashBeginCalled && tt.stubMode {
					t.Error("flashBegin should be called when data changed or skip disabled")
				}
			}
		})
	}
...
func TestSkipUnchangedOption(t *testing.T) {
	// Test that SkipUnchanged option is properly set
	opts := &FlasherOptions{
		SkipUnchanged: true,
	}

	if !opts.SkipUnchanged {
		t.Error("SkipUnchanged should be true")
	}

	opts2 := &FlasherOptions{
		SkipUnchanged: false,
	}

	if opts2.SkipUnchanged {
		t.Error("SkipUnchanged should be false")
	}
}

func TestSkipUnchangedInDefaultOptions(t *testing.T) {
	// Verify DefaultOptions initializes SkipUnchanged to false
	opts := DefaultOptions()
