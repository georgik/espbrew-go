...
func TestFlashBeginParamLength(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := &conn{
				port:                   mock,
				reader:                 newSlipReader(mock, context.Background()),
				stub:                   tt.isStub,
				supportsEncryptedFlash: tt.supportsEncr,
			}

			_ = c.flashBegin(4096, 0x1000, false, false) // ignore errors from mock
...
func TestFlashDeflBeginParamLength(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_ = c.flashDeflBegin(4096, 2048, 0x1000, false, false)
...
type mockConnection struct {
	...
	flashBeginFunc              func(size, offset uint32, encrypted bool, erase bool) error
	flashDataFunc               func(block []byte, seq uint32) error
	flashEndFunc                func(reboot bool) error
	flashDeflBeginFunc          func(uncompSize, compSize, offset uint32, encrypted bool, erase bool) error
...
func (m *mockConnection) flashBegin(size, offset uint32, encrypted bool, erase bool) error {
	if m.flashBeginFunc != nil {
		return m.flashBeginFunc(size, offset, encrypted, erase)
	}
	return nil
}
...
func (m *mockConnection) flashDeflBegin(uncompSize, compSize, offset uint32, encrypted bool, erase bool) error {
	if m.flashDeflBeginFunc != nil {
		return m.flashDeflBeginFunc(uncompSize, compSize, offset, encrypted, erase)
	}
	return nil
}
...
func TestConnFlushInputResetsPortAndReader(t *testing.T) {
	...
	c := newConn(port, context.Background())
