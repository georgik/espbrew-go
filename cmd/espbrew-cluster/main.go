...
func init() {
	rootCmd.Flags().StringVarP(&cfg.cfgFile, "config", "c", "", "Config file path")
	rootCmd.Flags().StringVarP(&cfg.role, "role", "r", "standalone", "Node role: master, worker, standalone")
	rootCmd.Flags().StringVar(&cfg.bindAddr, "bind", "0.0.0.0", "Bind address")
	rootCmd.Flags().IntVarP(&cfg.httpPort, "port", "p", 8080, "HTTP port")
	rootCmd.Flags().StringVar(&cfg.masterAddr, "master", "", "Master address (for workers)")
	rootCmd.Flags().StringVar(&cfg.logLevel, "log-level", "info", "Log level: debug, info, warn, error")
	rootCmd.Flags().IntVar(&cfg.workers, "workers", 2, "Number of flash workers")
	rootCmd.Flags().BoolVar(&cfg.disablemDNS, "no-mdns", false, "Disable mDNS discovery")
}
...
func runServer(cmd *cobra.Command, args []string) error {
	...
	// Fast shutdown: cancel context first
	cancel()

	// Stop executor quickly with timeout
